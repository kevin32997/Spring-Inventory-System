...
public class InventoryApiController {
    ...
    @GetMapping("api/inventory/view/page/{page}/{size}")
    private Response getInventoryByPage(@PathVariable("page") int page, @PathVariable("size") int size) {
        Pageable pageRequest = PageRequest.of(page - 1, size);
        Page<Inventory> inventoryPage = inventoryRepo.findAll(pageRequest);

        for(Inventory inv:inventoryPage.getContent()){
            try {
                Optional<Brand> optionalBrand = brandsRepo.findById(inv.getItem().getBrandId());
                inv.getItem().setBrandName(optionalBrand.get().getName());
            } catch (NoSuchElementException ex) {
                // do nothing or add action code
                inv.getItem().setBrandName("No Data found");
            }
            try {
                Optional<Type> optionalType = typesRepo.findById(inv.getItem().getTypeId());
                inv.getItem().setTypeName(optionalType.get().getName());
            } catch (NoSuchElementException ex) {
                // do nothing or add action code
                inv.getItem().setTypeName("No Data found");
            }
        }

        return Helper.createResponse("Request Successful", true, inventoryPage.getContent());
    }
    ...
    private Map<String, Long> getInventoryCount() {
        HashMap<String, Long> map = new HashMap<>();
        map.put("count", inventoryRepo.count());
        return map;
    }
