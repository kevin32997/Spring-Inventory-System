...
public class BrandsApiController {
    ...
    private Response addBrand(@ModelAttribute Brand brand) {
        try {
            Brand savedBrand = brandsRepo.save(brand);
            List<Brand> brandList = new ArrayList<>();
            brandList.add(savedBrand);

            return Helper.createResponse("Brand Saved.", true, brandList);
        } catch (Exception ex) {
    ...
    @GetMapping("api/brands/page/{page}/{size}/sort/{sortType}/{sortBy}")
    private Response getBrandsByPageSorted(
